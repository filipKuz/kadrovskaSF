...
public interface EmployeeJPARepository extends JpaRepository<Employee, Long> {
	...
	@Query("select e from Employee e" + " where (LOWER(e.firstName) like (CONCAT( '%', LOWER(:searchTerm), '%'))"
			+ " or LOWER(e.lastName) like (CONCAT( '%', LOWER(:searchTerm), '%'))"
			+ " or LOWER(e.birthDate) like (CONCAT( '%', LOWER(:searchTerm), '%')))")
	...
	@Query("select e from Employee e where e.city.cityId = :cityId")
	List<Employee> findByCityId(@Param("cityId") long cityId);
}
