...
public class VacationReqController {
	...
	public ResponseEntity<List<VacationRequestDTO>> getVReqbyAHRDD(
			@PathVariable("ahrId") Long ahrId) {

		return new ResponseEntity<>((toVReqDTO.convert(vacationReqService.findByAnnualHolidayRegulationAnnualHolidayRegulationId(ahrId))),
				HttpStatus.OK);
	}
	
	@GetMapping(value="/findById/{id}")
	public ResponseEntity<VacationRequestDTO> getById(
	...
	public ResponseEntity<String> deleteVReq(@PathVariable("id") Long id) {
		...
		AnnualHolidayRegulation ahr = ahrService.findOne(vreq.getAnnualHolidayRegulation().getAnnualHolidayRegulationId());
